(ns audio.loop
  ...
  through audio.cosci -> audio.governor -> commit+publish. `run!` reduces
  ...
  asset according to WHICHEVER modality actually won — no special-casing
  needed in polling/settling itself, since murakumo.poll works the same
  ...
  (:require [clojure.java.io :as io]
            ...
            [audio.judge :as judge]
            [audio.cosci :as cosci]
            [audio.governor :as governor]
            [audio.datalad :as datalad]
            [audio.ledger :as ledger]
            [audio.publish :as publish]
            [audio.aozora :as aozora]))

(def lease-path ".audio/lease")
(def state-path ".audio/state.edn")
...
(defn submit-round!
  ...
  logs to the ledger and continues with however many did submit). Each
  candidate carries its own :modality (:music/:sfx) from
  audio.generate/round-candidates, which audio.murakumo/submit! resolves to
  the right murakumo function."
  [state]
...
(defn- fetch-result
  "One pending entry -> a audio.cosci/reflect-shaped result map, or nil if
...
(defn- asset-title
  "Winning candidate's gene -> a short human title. Gene shape differs by
  modality (:music = {:mood :tempo :instrumentation}, :sfx = {:kind
  :character :length}), so this branches on the winner's own :modality."
  [modality gene]
  (case modality
    :music (str/join ", " (remove nil? [(:mood gene) (:instrumentation gene)]))
    :sfx   (str/join ", " (remove nil? [(:kind gene) (:character gene)]))
    (str/join ", " (remove nil? (vals gene)))))
   ...
   (let [p (persona/load-persona)
     ...
     (when winner
       (let [{:keys [result score]} winner
             ...
             asset {:id (str "audio-" (str/replace job-id #"[^a-zA-Z0-9-]" "-"))
                    :kind :asset :format format :artifact-bytes artifact-bytes
                    :title (asset-title modality (:gene candidate))
           ...
           (let [manifest (datalad/->isekai-manifest (assoc asset :kind (:persona/kind p)))
             ...
             (let [pub (or publisher (aozora/aozora-publisher
               ...
               (ledger/append! {:t :accepted :round n :candidate-id job-id :score score
                                :modality modality :asset-path asset-path :published published}))))))
